jQuery(function($) {
  ...
  $(document).on('click', '#gallery-metabox a.change-image', function(e) {
    ...
    file_frame.on( 'select', function() {
      ...
      that.parent().find('img.image').attr('src', attachment.sizes.thumbnail.url);
    });
  ...
  function resetIndex() {
    $('#gallery-metabox-list li').each(function(i) {
      $(this).find('input:hidden').attr('name', 'DREAM_auto_slider[' + i + ']');
    });
  }
